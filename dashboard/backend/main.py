...
async def register_device(data: DeviceRegister):
    try:
        ...
        if not exists.data:
            ...
            print(f"🆕 Device Registered: {data.mac_address} | Key: {data.mac_key}")
            ...
            print(f"🔄 Device Reconnected: {data.mac_address}")
...
async def log_hardware_data(data: SensorData):
    try:
        ...
        print(f"📦 Logging Data for User {user_id}: {payload}")
...
def get_weather():
    try:
        ...
        try:
            ...
            print(f"⚠️ Weather DB Log failed: {db_err}")
